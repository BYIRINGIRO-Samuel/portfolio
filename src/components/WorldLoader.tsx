...
const WorldLoader = ({ onComplete }: { onComplete?: () => void }) => {
  ...
  useEffect(() => {
    // Accelerated HUD progress for 5s total duration
    const hudInterval = setInterval(() => {
        setHudProgress(prev => {
            if (prev >= 100) return 100;
            return Math.min(100, prev + 4); // Adjusted for 5s
        });
    }, 150);
    ...
    const finishTimer = setTimeout(() => {
        setIsDone(true);
        setHudProgress(100);
    }, 4000); // Mark done earlier

    const exitTimer = setTimeout(() => {
      setIsExiting(true);
      if (onComplete) setTimeout(onComplete, 1000);
    }, 5000); // Total scene duration: 5s
              ...
              <div className="absolute inset-0 origin-center scale-[0.42] translate-x-[-150px] translate-y-[100px]">
                 ...
                 />
                  ...
                  <motion.div 
                    ...
                    transition={{ offsetDistance: { duration: 4, ease: "easeInOut" } }}
                    style={{ 
                        offsetPath: `path("${roadPath}")`,
                        offsetRotate: "0deg",
                  ...
                  <motion.div 
                    ...
                    transition={{ 
                      offsetDistance: { duration: 4, ease: "easeInOut" },
                      filter: { duration: 2.5 }
                    }}
                    style={{ 
                      offsetPath: `path("${roadPath}")`,
