...
const ProjectsSection = () => {
  return (
    <section id="projects" className="bg-[#050505] text-white py-12 flex flex-col items-center overflow-hidden font-sans selection:bg-white/30">
      
      {/* SECTION HEADER: UNIFIED HUD (Reduced Size) */}
      <div className="max-w-[1240px] w-full px-8 mb-10 flex items-end justify-between">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-[1240px] w-full px-8">
        {allProjects.map((p) => (
          <div key={p.id} className="flex flex-col items-center gap-6">
            
            {/* THE NFT CARD: 100% REPLICA ARCHITECTURE (Tightened) */}
            <div className="w-full bg-[#111111] rounded-[20px] p-2.5 pb-5 border border-white/5 relative overflow-hidden transition-all duration-500 shadow-2xl">
              ...
              <div 
                className="relative mx-auto w-[94%] aspect-square rounded-[10px] rounded-tr-[36px] rounded-bl-[36px] overflow-hidden flex items-center justify-center group/img bg-[#1a1a1a]"
              >
                <img 
                  src={p.image} 
                  alt={p.name} 
              ...
              {/* DESCRIPTION PILL */}
              <div className="mt-4 mx-auto w-[94%] bg-[#080808] border border-white/5 rounded-lg p-3.5 flex items-center gap-3">
                 ...
                 <span className="text-[11px] font-bold text-white/50 italic leading-tight uppercase tracking-tight line-clamp-1">
                   {p.shortDesc}
                 </span>
              </div>
      ...
      {/* VIEW ALL ARCHIVE (Tightened) */}
      <div className="mt-14">
         <button className="group relative flex flex-col items-center gap-3">
            <div className="flex items-center gap-3 px-12 py-4 bg-white text-black font-black text-[11px] tracking-[0.5em] uppercase italic transition-all hover:bg-gray-200 shadow-xl">
               VIEW_ARCHIVE <ArrowRight className="w-3.5 h-3.5" />
            </div>
            <div className="flex items-center gap-1.5 opacity-20">
               <ChevronDown className="w-2.5 h-2.5 animate-bounce" />
               <span className="text-[8px] font-black tracking-widest uppercase italic">Archive_Size: 48</span>
            </div>
         </button>
      </div>
